...
# 
### BEGIN INIT INFO
# Provides:          provisionadmin-service
...
PATH=/usr/local/sbin:/usr/local/bin:/sbin:/bin:/usr/sbin:/usr/bin:/var/app/enabled/provisionadmin-service
...
function start_uwsgi()
{
	if [ -f "$PID_FILE" ]; then
	    ...
	    pushd ${APP_DIR}/provisionadmin >/dev/null
	    #uwsgi --pidfile=${PID_FILE} -x uwsgi.xml --uid dolphinopadmin --gid nogroup
	    uwsgi --pidfile=${PID_FILE} --ini uwsgi.cfg --uid dolphinopadmin --gid nogroup
	    popd >/dev/null	
	fi
}
...
case "$1" in
	...
	status)
		status_of_proc -p /var/run/provisionadmin-service-uwsgi.pid "$DAEMON" uwsgi && exit 0 || exit $?
		;;
	*)
